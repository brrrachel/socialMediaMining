    ...
    try:
        csvWriter.writerow(
            ['id', 'conversation_id', 'created_at', 'date', 'time', 'timezone', 'user_id', 'username', 'name', 'place',
             'tweet', 'mentions', 'urls', 'photos', 'replies_count', 'retweets_count', 'likes_count', 'hashtags',
             'cashtags', 'link', 'retweet', 'quote_url', 'video', 'near', 'geo', 'source', 'user_rt_id', 'user_rt',
             'retweet_id', 'reply_to', 'retweet_date'])

        # match all details on our needed columns or empty columns which can't be determined in tweepy but given in twint
        for tweet in tweepy.Cursor(api.user_timeline, id=user, tweet_mode='extended').items():
            ...
            retweet_date = ''
            csvWriter.writerow(
                [tweet_id, conversation_id, createdAtDate, date, time, timezone, user_id, username, name, place,
                 tweetTextNoEmojis, mentions, urls, photos, replies_count, retweets_count, likes_count, hashtags,
                 cashtags, link, retweet, quote_url, video, near, geo, source, user_rt_id, user_rt, retweet_id,
