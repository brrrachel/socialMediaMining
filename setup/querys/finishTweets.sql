INSERT INTO tweets (id, account_id, conversation_id, created_at, date, time, timezone, user_id, place, tweet, mentions,
    ...
    SELECT * from (select t.id::bigint, a.id, t.conversation_id::bigint, (t.date::date + t.time::time) as created_at,
        ...
        FROM tmp_tweets t join accounts a on '@'||upper(t.username)=upper(a.name)) final;
